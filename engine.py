...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for images, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        images, targets, texts, aux_texts = prepare_inputs(images, targets, data_loader, device)
        outputs = model(images.tensors, texts, aux_texts, images.mask)
        loss_dict, indices = criterion(outputs, targets, aux_texts)
...
def evaluate(model, postprocessors, criterion, data_loader, device, args):
    ...
    for images, targets in metric_logger.log_every(data_loader, 10, header):
        ...
        logits_per_hoi = model.logit_scale.exp() * hoi_text_similarity
...
def prepare_inputs(images, targets, data_loader, device):
    ...
    object_token = [torch.as_tensor([sot_token] + _tokenizer.encode(i) + [eot_token], dtype=torch.long).to(device) for i in text_inputs]
    object_index = torch.tensor([text_inputs.index(i) for i in [hoi['text'][-1].replace("_", " ") for t in targets for hoi in t['hois']]], device=device)
    ...
    return images, targets, texts, aux_texts
