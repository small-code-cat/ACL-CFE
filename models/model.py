...
class HOIVisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.my_person = MLP(output_dim, output_dim*2, output_dim, 3)
        self.my_action = MLP(output_dim, output_dim * 2, output_dim, 3)
        self.my_object = MLP(output_dim, output_dim*2, output_dim, 3)
    ...
    def initialize_parameters(self):
        ...
        for layer in self.my_person.layers:
            nn.init.xavier_uniform_(layer.weight, gain=1)
            nn.init.constant_(layer.bias, 0)
        for layer in self.my_action.layers:
            nn.init.xavier_uniform_(layer.weight, gain=1)
            nn.init.constant_(layer.bias, 0)
        for layer in self.my_object.layers:
            nn.init.xavier_uniform_(layer.weight, gain=1)
            nn.init.constant_(layer.bias, 0)
    ...
    def forward(self, image: torch.Tensor, mask: torch.Tensor = None):
        ...
        person_features = self.my_person(hoi_features)
        action_features = self.my_action(hoi_features)
        object_features = self.my_object(hoi_features)
...
class HOIDetector(nn.Module):
    ...
    def encode_text(self, text):
        ...
        x, eot_indices = self.text_to_embedding(text) if type(text)==list else self.aux_text_to_embedding(text)
    ...
    def aux_text_to_embedding(self, text):
        all_token_embeddings = []
        eot_indices = []
        text = list(text.values())[0]
        for i in text:
            remain_length = self.context_length - len(i)
            eot_indices.append(self.context_length - remain_length - 1)
            padding_zeros = torch.zeros(remain_length, dtype=torch.long).to(i.device)
            token = torch.cat([i, padding_zeros])
    ...
    def forward(self, image, text, aux_texts, image_mask=None):
        # vision encoder
        vision_outputs = self.encode_image(image, image_mask)

        # text encoder
        text_features = self.encode_text(text)
        aux_action_text_features = self.encode_text({'action_token': aux_texts['action_token']})
        aux_object_text_features = self.encode_text({'object_token':aux_texts['object_token']})
        ...
        person_features = vision_outputs["person_features"]
        person_features = person_features / person_features.norm(dim=-1, keepdim=True)
        action_features = vision_outputs["action_features"]
        action_features = action_features / action_features.norm(dim=-1, keepdim=True)
        object_features = vision_outputs["object_features"]
        object_features = object_features / object_features.norm(dim=-1, keepdim=True)
        text_features = text_features / text_features.norm(dim=-1, keepdim=True)
        aux_object_text_features = aux_object_text_features / aux_object_text_features.norm(dim=-1, keepdim=True)
        aux_action_text_features = aux_action_text_features / aux_action_text_features.norm(dim=-1, keepdim=True)
        ...
        hoi_text_similarity = hoi_features @ text_features.t()
        logits_per_hoi = logit_scale * hoi_text_similarity
        logits_per_hoi_person = logit_scale * person_features @ aux_object_text_features.t()
        logits_per_hoi_action = logit_scale * action_features @ aux_action_text_features.t()
        logits_per_hoi_object = logit_scale * object_features @ aux_object_text_features.t()

        return_dict = {
            "logits_per_image": logits_per_image,
            "logits_per_text": logits_per_text,
            "logits_per_hoi": logits_per_hoi,
            'hoi_text_similarity': hoi_text_similarity,
            'logits_per_hoi_person':logits_per_hoi_person,
            'logits_per_hoi_action':logits_per_hoi_action,
            'logits_per_hoi_object':logits_per_hoi_object,
