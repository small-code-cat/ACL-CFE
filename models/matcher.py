...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets, aux_texts=None):
        ...
        if aux_texts is not None:
            out_person_prob = outputs["logits_per_hoi_person"].flatten(0, 1).softmax(-1)
            out_action_prob = outputs["logits_per_hoi_action"].flatten(0, 1).softmax(-1)
            out_object_prob = outputs["logits_per_hoi_object"].flatten(0, 1).softmax(-1)
        ...
        if aux_texts is not None:
            cost_person_class = -out_person_prob[:, 0].unsqueeze(-1)
            cost_action_class = -out_action_prob[:, aux_texts['action_index']]
            cost_object_class = -out_object_prob[:, aux_texts['object_index']]
        ...
        if aux_texts is not None:
            C = self.cost_bbox * cost_pbbox + self.cost_bbox * cost_obbox + \
                ...
                self.cost_class * cost_person_class + self.cost_class * cost_object_class + self.cost_class * cost_action_class
        else:
            C = self.cost_bbox * cost_pbbox + self.cost_bbox * cost_obbox + \
                self.cost_giou * cost_pgiou + self.cost_giou * cost_ogiou + \
                self.cost_class * cost_class + self.cost_conf * cost_conf
