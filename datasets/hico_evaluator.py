...
class HICOEvaluator(object):
    ...
    def accumulate(self):
        ...
        for hoi_id in range(600):
            ...
            ap, rec, hit_dict = calc_ap(self.scores[hoi_id], self.boxes[hoi_id], self.keys[hoi_id], gts_per_hoi, hit_dict)
            self.hico_ap[hoi_id], self.hico_rec[hoi_id] = ap, rec
        with open("result_hico.pkl", "wb") as f:
...
def calc_ap(scores, boxes, keys, gt_boxes, hit_dict):
    ...
    return ap, np.max(rec) if len(rec) else 0, hit_dict
